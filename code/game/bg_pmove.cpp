	...
	{//in air or submerged in water or in ladder
		...
		if ( pm->waterlevel > 0 )
		{
			...
			{//FIXME: check for watertype, save waterlevel for whether to play
				...
				{
					PM_SetAnim( pm, SETANIM_LEGS, /*BOTH_LADDER_IDLE*/FACE_ALERT, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD|SETANIM_FLAG_RESTART );
					pm->ps->legsAnimTimer += 300;
					if ( pm->waterlevel >= 2 )	//arms on ladder
					{
						PM_SetAnim( pm, SETANIM_TORSO, /*BOTH_LADDER_IDLE*/FACE_ALERT, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD|SETANIM_FLAG_RESTART );
