...
void Shark_SetBolts( gentity_t* self )
{
	if ( self && self->client )
	{
...
qboolean Shark_CheckRoar(gentity_t* self)
{
	if (!self->wait)
...
-------------------------
*/
void Shark_Move(qboolean visible)
{
...
void Shark_DropVictim(gentity_t* self)
{
	//FIXME: if Shark dies, it should drop its victim.
	//FIXME: if Shark is removed, it must remove its victim.
	if (self->activator)
	{
...
void Shark_Grab(qboolean tryGrab)
{
...
void Shark_Attack(float distance, qboolean doCharge)
{
...
void Shark_Combat(void)
{
...

	{
		...
		{
			Shark_Attack(distance, doCharge);
		}
	}
}
...
-------------------------
*/
void NPC_Shark_Pain(gentity_t* self, gentity_t* attacker, int damage)
{
	qboolean hitByShark = qfalse;
	if (attacker && attacker->client && attacker->client->NPC_class == CLASS_SHARK)
	{
		hitByShark = qtrue;
	}
	if (attacker
		...
		&& !(attacker->flags & FL_NOTARGET))
	{
		if (!self->count)
		{
			if ((!attacker->s.number && !Q_irand(0, 3))
				|| !self->enemy
				|| self->enemy->health == 0
				|| (self->enemy->client && self->enemy->client->NPC_class == CLASS_SHARK)
			...
			{//if my enemy is dead (or attacked by player) and I'm not still holding/eating someone, turn on the attacker
				...
				if (hitByShark)
				{//stay mad at this Shark for 2-5 secs before looking for attacker enemies
					TIMER_Set(self, "sharkInfight", Q_irand(2000, 5000));
				}

			}
		}
	}
	if ((hitByShark || (self->count == 1 && self->activator && !Q_irand(0, 4)) || Q_irand(0, 200) < damage)//hit by shark, hit while holding live victim, or took a lot of damage
		&& self->client->ps.legsAnim != BOTH_STAND1TO2
		&& TIMER_Done(self, "takingPain"))
	{
		if (!Shark_CheckRoar(self))
		{
...
void Shark_CheckDropVictim(void)
{
	vec3_t mins, maxs;
	vec3_t start, end;
	trace_t	trace;
...
void NPC_BSShark_Default(void)
{
	...
	if (NPCS.NPC->enemy)
	{
		...
		if ( NPC->enemy->client //enemy is a client
			...
			&& (!NPC->enemy->enemy || !NPC->enemy->enemy->client || NPC->enemy->enemy->client->NPC_class!=CLASS_SHARK) )//enemy's enemy is not a client or is not a shark (which is as scary as me anyway)
		...
		*/
		...
		if (NPCS.NPC->enemy->client && NPCS.NPC->enemy->client->NPC_class == CLASS_SHARK)
		{//got mad at another Shark, look for a valid enemy
			if (TIMER_Done(NPCS.NPC, "sharkInfight"))
			{
