...
void RB_BeginDrawingView(void) {
	...
	if (tr.refdef.rdflags & RDF_AUTOMAP || (!(backEnd.refdef.rdflags & RDF_NOWORLDMODEL) && (r_dynamicGlow->integer)))
	{
		if (tr.world && tr.world->globalFog)
...
const void *RB_ExportCubemaps(const void *data)
{
	...
	if (cmd)
	{
		...
		byte *cubemapPixels = (byte *)Z_Malloc(sideSize * 6, TAG_TEMP_WORKSPACE);
		int i, j;
...
const void *RB_BuildSphericalHarmonics(const void *data)
{
	...
	if (cmd)
	{
		...
		cubemap_t *currentSH = (cubemap_t *)Z_Malloc(sizeof(*tr.cubemaps), TAG_TEMP_WORKSPACE);
		currentSH[0].image = bufferImage;
		int buildedSphericalHarmonics = 0;

		float *cubemapPixels = (float *)Z_Malloc(sideSize * sizeof(float), TAG_TEMP_WORKSPACE);
